...
Write-Host "=== AI Sales Agent Platform v0.14.2 - repository hygiene/package build hotfix ===" -ForegroundColor Cyan
...
Write-Host "`n[2/6] Starting v0.14.2 API (sem nova migration)..." -ForegroundColor Yellow
docker compose up -d api
if ($LASTEXITCODE -ne 0) { throw "Falha ao iniciar API v0.14" }

Write-Host "`n[3/6] Waiting for readiness v0.14.2..." -ForegroundColor Yellow
$healthy = $false
for ($i = 0; $i -lt 60; $i++) {
    try {
        $h = Invoke-RestMethod -Uri "http://localhost:8000/api/v1/health/ready" -TimeoutSec 3
        if (($h.status -eq "ok") -and ($h.version -eq "0.14.2")) { $healthy = $true; break }
    } catch {}
    Start-Sleep -Seconds 2
}
if (-not $healthy) {
    docker compose logs --tail=200 api
    throw "API v0.14.2 did not become ready"
}

Write-Host "`n[4/6] Full regression suite - alvo 52 testes..." -ForegroundColor Yellow
docker compose exec -T api pytest -q tests
if ($LASTEXITCODE -ne 0) { throw "Regressao v0.14.2 falhou" }

Write-Host "`n[5/6] Package discovery + Ruff + supply-chain contract..." -ForegroundColor Yellow
docker compose exec -T api sh -lc 'rm -rf /tmp/v0142-src /tmp/v0142-wheel && mkdir -p /tmp/v0142-src /tmp/v0142-wheel && cp pyproject.toml README.md /tmp/v0142-src/ && for d in app alembic fixtures observability scripts tests; do if [ -e "$d" ]; then cp -a "$d" /tmp/v0142-src/; fi; done && cd /tmp/v0142-src && python -m pip wheel --no-deps --no-build-isolation . -w /tmp/v0142-wheel'
...
if ($LASTEXITCODE -ne 0) { throw "Supply-chain contract v0.14.2 falhou" }
...
if ($LASTEXITCODE -ne 0) { throw "Build production v0.14.2 falhou" }
$prodUser = docker image inspect ai-sales-agent-platform:v0.14.2 --format '{{.Config.User}}'
...
Write-Host "`n=== v0.14.2 HOTFIX APPLIED ===" -ForegroundColor Green
Write-Host "Regression : 52 testes esperados" -ForegroundColor Green
Write-Host "Production : ai-sales-agent-platform:v0.14.2 (USER app)" -ForegroundColor Green
