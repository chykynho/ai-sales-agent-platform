...
Write-Host "=== AI Sales Agent Platform v0.14 - Full Quality/Supply Chain Gate ===" -ForegroundColor Cyan

$artifactDir = Join-Path (Get-Location) "artifacts\v0.14"
...
$imageTar = Join-Path $artifactDir "ai-sales-agent-platform-v0.14.0.tar"
docker save ai-sales-agent-platform:v0.14.0 -o "$imageTar"
if ($LASTEXITCODE -ne 0) { throw "docker save falhou" }
$artMount = "$($artifactDir):/artifacts"
docker run --rm -v "$artMount" aquasec/trivy:0.73.0 image --input /artifacts/ai-sales-agent-platform-v0.14.0.tar --severity CRITICAL --ignore-unfixed --exit-code 1 --no-progress
if ($LASTEXITCODE -ne 0) { throw "Trivy encontrou vulnerabilidade CRITICAL corrigivel" }

Write-Host "`n=== v0.14 QUALITY/SUPPLY CHAIN VALIDADA COM SUCESSO ===" -ForegroundColor Green
