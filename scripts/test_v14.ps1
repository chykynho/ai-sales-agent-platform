...
Write-Host "=== AI Sales Agent Platform v0.14.1 - Full Quality/Supply Chain Gate ===" -ForegroundColor Cyan

$artifactDir = Join-Path (Get-Location) "artifacts\v0.14.1"
...
$imageTar = Join-Path $artifactDir "ai-sales-agent-platform-v0.14.1.tar"
docker save ai-sales-agent-platform:v0.14.1 -o "$imageTar"
if ($LASTEXITCODE -ne 0) { throw "docker save falhou" }
$artMount = "$($artifactDir):/artifacts"
docker run --rm -v "$artMount" aquasec/trivy:0.73.0 image --input /artifacts/ai-sales-agent-platform-v0.14.1.tar --severity CRITICAL --ignore-unfixed --exit-code 1 --no-progress
if ($LASTEXITCODE -ne 0) { throw "Trivy encontrou vulnerabilidade CRITICAL corrigivel" }

Write-Host "`n=== v0.14.1 QUALITY/SUPPLY CHAIN VALIDADA COM SUCESSO ===" -ForegroundColor Green
