...
Write-Host "=== AI Sales Agent Platform v0.12 - Start Observability Stack ===" -ForegroundColor Cyan
...
for($i=0; $i -lt 60; $i++) {
    ...
    try { if((Invoke-RestMethod "http://localhost:8000/api/v1/health/ready" -TimeoutSec 2).status -eq "ok"){$apiOk=$true} } catch {}
...
docker compose @compose ps
Write-Host "`n=== OBSERVABILITY STACK READY ===" -ForegroundColor Green
...
Write-Host "`nNext: PowerShell -ExecutionPolicy Bypass -File .\scripts\test_observability_stack_v12.ps1"
